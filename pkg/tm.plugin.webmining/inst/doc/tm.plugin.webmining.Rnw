...
<<Init_shown,echo=TRUE, eval=F>>=
library(tm.plugin.webmining)
@ 

\section{Package Overview}
<<dependenceplot, fig=T,echo=F>>=
library(diagram)
lables <- c("tm.plugin.webmining", "tm", "boilerpipeR",  "RCurl" ,"XML", "RJSONIO")
par(mar=c(0,0,0,0))
plot.new()
...
textrect (elpos[1,],0.2,0.05,lab=lables[1],cex=1, , shadow.size = 0, font = 2)
for ( i in 2:6){
	textrect (elpos[i,],0.1,0.05,lab=lables[i],cex=1, shadow.size = 0)
} 
@
\begin{figure}[tbp]
\centering
