...
main content from \proglang{HTML} files, removing unwanted elements for text
mining purposes like ads, banners, headers and footers.
}
...
library(RCurl)
@
we can retrieve the content of a webpage using \pkg{RCurl}:
<<eval=F, echo = T>>=
